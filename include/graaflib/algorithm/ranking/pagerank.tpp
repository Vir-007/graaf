...
std::optional<std::unordered_map<vertex_id_t, double>> pagerank(
  ...
  // Ranks are indexed directly by vertex_id_t rather than keyed in an
  // unordered_map: every edge is followed once per iteration, so a hash
  // lookup per edge dominated the runtime. Every vertex is known up front,
  // so the vectors are sized once to the largest id. Removed vertices can
  // leave gaps below that id, which is why only the live ids collected here
  // are ever iterated.
